...
import { useScrollPosition, useDeviceOptimization } from '../../hooks/useVirtualization'
...
interface VirtualMemoGridProps {
...
const Cell: React.FC<CellProps> = ({ columnIndex, rowIndex, style, data }) => {
  const { memos, columnCount, onEditMemo, onDeleteMemo } = data
...
const VirtualMemoGrid: React.FC<VirtualMemoGridProps> = ({
  onEditMemo,
  onDeleteMemo,
  onCreateMemo,
  isLoading = false,
  ...
  const { optimizationSettings } = useDeviceOptimization()
  ...
  const getColumnCount = useCallback(() => {
    ...
    if (width < 640) return 1        // sm未満
    if (width < 1024) return 2       // lg未満
    if (width < 1280) return 3       // xl未満
    return 4                         // xl以上
  }, [])
  ...
  useEffect(() => {
    ...
    if (gridRef.current && (restored.scrollTop > 0 || restored.scrollLeft > 0)) {
  ...
  // グリッド計算
  const rowCount = Math.ceil(memos.length / columnCount)
  const cardHeight = 192 + 24 // カード高さ(192px) + パディング(24px)
  const cardWidth = useMemo(() => {
    if (typeof window === 'undefined') return 300
    
    const containerWidth = window.innerWidth - 48 // パディング考慮
    const gapWidth = (columnCount - 1) * 24        // gap-6 = 24px
    return (containerWidth - gapWidth) / columnCount
  }, [columnCount])
  ...
  const cellData = useMemo(() => ({
    ...
    onEditMemo,
    onDeleteMemo,
  }), [memos, columnCount, onEditMemo, onDeleteMemo])

  // コンテナサイズ計算
  const containerHeight = Math.min(
    typeof window !== 'undefined' ? window.innerHeight - 200 : 600, // ヘッダー等を除いた高さ
    rowCount * cardHeight
  )
    ...
    <div className="p-6">
      ...
      <div className="virtual-grid-container">
        <Grid
          ...
          itemData={cellData}
          overscanRowCount={optimizationSettings.overscanRowCount}
          overscanColumnCount={optimizationSettings.overscanColumnCount}
          initialScrollTop={scrollPosition.scrollTop}
          initialScrollLeft={scrollPosition.scrollLeft}
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-4 text-xs text-gray-500 text-center">
          仮想化: {memos.length}件中 約{Math.min(
            (Math.ceil(containerHeight / cardHeight) + 4) * columnCount,
            memos.length
          )}件をレンダリング
        </div>
      )}
...
export default VirtualMemoGrid