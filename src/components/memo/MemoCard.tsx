...
const MemoCard: React.FC<MemoCardProps> = ({ memo, onEdit, onDelete }) => {
  ...
  return (
    <div className="memo-card-container perspective-1000 h-48 cursor-pointer group">
      ...
      >
        {/* 表面 */}
        <div className={`card-face absolute w-full h-full backface-hidden bg-gradient-to-br ${frontColorClasses} rounded-lg p-4 flex flex-col justify-between text-white shadow-lg hover:shadow-xl transition-shadow`}>
          {/* 表面コンテンツ */}
          <div className="flex-1 overflow-hidden">
            ...
            <div className="text-lg font-semibold leading-tight">
              {memo.frontContent ? truncateText(memo.frontContent, 80) : 'タイトルを入力してください'}
            </div>
          </div>
        ...
        {/* 裏面 */}
        <div className={`card-face absolute w-full h-full backface-hidden rotate-y-180 bg-gradient-to-br ${backColorClasses} rounded-lg p-4 flex flex-col justify-between text-white shadow-lg`}>
          {/* 裏面コンテンツ */}
          <div className="flex-1 overflow-hidden">
            <div className="text-sm leading-relaxed">
              {memo.backContent ? truncateText(memo.backContent, 150) : '内容を入力してください'}
            </div>
          </div>
